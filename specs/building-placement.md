...
> Status: **✅ ALL FIVE SLICES BUILT AND MERGED TO `main`** — the singleton seam, place-a-building, the residential brush, **the harvest brush** (D87, D112–D130) and **the planting brush** (D125, D126, D146, D220). · Owner: Joe + Claude Code
> ⚠️ *Corrected 2026-08-28: this said slices 4 and 5 "remain" long after both shipped — `HarvestBrushTests.cs`, `PlantedSaplingTests.cs` and the Harvest/Planting rows in the build bar all exist. §13's own slice list still carries no ✅ on either.*
...
- **~~It keeps `MaxHomeToWorkTiles` a real guarantee~~** — ⚠️ a budget since D120, and since D381 not even a search radius: `ChooseSite` walks every whole-painted tile in the valley and scores the walk. `Household.ChooseSite` survives intact — it is simply *constrained to the zone* instead of the whole valley. The sim still picks the best spot it can; the player decides the neighbourhood.
...
**2. Placing a building. ✅ Built 2026-07-28, both halves** — the sim, then a build menu, a ghost under the cursor and `CanBuildAt`'s refusals shown as words.** Granary, warehouse, market, woodcutter's hut. Construction sites are `Workplace`s of kind `Builder`, so they inherit allocation, catchment and refusal reasons rather than growing a parallel system — and the job disappears when the building exists. Materials are hauled from the nearest warehouse that has logs; work cannot start until they arrive. Building is funded from spare hands and **yields first**, alongside cutting logs for houses.
   ...
   - ⚠️ **Half of the first sentence has since been overturned (D110).** A site is still a `Workplace` of kind `Builder` — that is what carries its position, its name and its place in the build queue — but **nobody is assigned to one any more.** Builders hold their job at a **builder's hut** the player places, and a site is an errand its crew walks out to, taken from the head of the queue. So a site has no seats, takes no workers, and is skipped by the allocator; `construction_site_capacity` is deleted. What it still inherits is catchment-free reach and the queue; what it no longer inherits is allocation. **And there is no fallback: no hut, nothing is raised.**
...
**3. ~~The residential brush.~~ ✅ Done 2026-07-28.** Zones are sim state — hashed, deterministic, part of the seed contract, because a zone is a decision somebody made. `Household.ChooseSite` is unchanged except that it only looks at painted land: **the player picks the neighbourhood, the sim still picks the tile**, since it knows the walk to work and the walk to the store and a cursor does not.
   ...
   - ⛔ **~~Erasing land says where the village may build *next*; houses already standing stay put. Pulling homes down because somebody adjusted a brush would be a cruel reading of an undo.~~ OVERTURNED by D228, annotated 2026-08-28.** Unpainting now **marks a standing house for demolition** (`SimWorld.EraseResidential` calls `MarkDemolition`). ⭐ **D228 quoted this exact sentence when it overturned it and the spec was never touched** — and D230 then softened *level-instantly* to *mark, with repainting cancelling it right up until the first hammer swings*, which is what makes the objection above no longer apply: **a brush wobble costs nothing, so there is nothing cruel left to guard against.** *(D235 then deleted the confirmation click that had existed only for the one commit where levelling was instant.)*
