...
> ✅ **THAT DAY CAME — D358, 2026-09-11.** Worn ground is cheaper (`specs/desire-paths.md`), and the priced field is **Dial's algorithm** (`TerrainCostField.Refill`): a ring of `baseTileCost + 1` buckets, O(n + total cost), the same answer as the sweep on uniform ground byte for byte (`OnUniformGroundDijkstraIsTheSweep`). ⚠️ *The `PriorityQueue` this line prescribed was built first and measured — it cost the suite a minute; the bucket queue is within a whisker of the sweep.* The sweep is still used, over reused buffers, for a valley nobody has worn.
