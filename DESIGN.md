...
**Current phase:** **Phase 2 (branch `phase/2-wood-fuel-and-tools`)**, re-ordered by Joe's call — §4 invites that. Wood-as-fuel (D17/D29) was taken before the environment work because winter needed a second axis to bite on, and storage (D30/D32) after it because every goods bug so far has been "the right stuff in the wrong place". Environment/seasons depth + biomes (§2.5) is still the phase's headline and is not started.
...
**In progress:**
- **`specs/storage-and-distribution.md` slices 4 and 5** — the manned market, and storage capacity as a binding constraint. Both are specified and neither is started.

**Next up:**
- **Whether a peak-and-settle population curve is acceptable** (Joe's call, asked and open). The village survives 150 years and most deaths are old age, but it peaks near 75 and settles around 20 rather than holding flat, and D31 says "stable size". A flat line probably means growth stopping at what the buildings support instead of overshooting them first — which points at slice 5, capacity, since capacity is what the village keeps outgrowing.
