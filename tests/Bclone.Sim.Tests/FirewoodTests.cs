...
public sealed class FirewoodTests
{
    ...
    public void AWoodcutterWithNoLogsSaysSo()
    {
        ...
        for (int i = 0; i < Config.TicksPerYear * 30 && note is null; i++)
        {
            ...
            foreach (Household household in loop.World.Households)
            {
                ...
                loop.World.AnyStoreOf(StoreKind.Warehouse).Store.TryTake(Goods.Logs, loop.World.AnyStoreOf(StoreKind.Warehouse).Store.Logs);
            }
        ...
        // warehouse has no logs"; with more than one warehouse possible (D38) that
        // sentence would have been unverifiable by the player — WHICH warehouse? — so it now
        // says no warehouse within reach has them.
    ...
    public void TheChainDoesNotStarveInTheMiddle()
    {
        ...
        for (int season = 1; season <= 60 * 4; season++)
        {
            ...
            checkedSeasons++;
            Assert.True(quota.Foresters > 0 || loop.World.AnyStoreOf(StoreKind.Warehouse).Store.Logs >= config.LogsPerSplit,
                $"Season {season}: the village wants {quota.Woodcutters} woodcutters but no " +
                $"foresters, and only {loop.World.AnyStoreOf(StoreKind.Warehouse).Store.Logs} logs in store — {quota}");
        }
